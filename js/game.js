const game = {
    ...
    createBackground() {
        this.background = new Background(this.ctx, this.gameSize.w, this.gameSize.h + 60)
    },
    createBall() {
    ...
    platformCollision(elm) {
        ...
        if (this.player.playerPos.y + this.player.playerSize.h <= elm.platformPos.y &&
            ...
            this.player.playerPos.x + this.player.playerSize.w >= elm.platformPos.x &&
            this.player.playerPos.x <= elm.platformPos.x + elm.platformSize.w) {


            this.player.playerSpeed.y = 0
            this.player.playerBaseline = elm.platformPos.y


        }
    },

    enemy1HurtCollision(elm) {
        if (this.player.playerPos.y + this.player.playerSize.h <= elm.enemy1Pos.y + 1 &&
            this.player.playerPos.y + this.player.playerSize.h + this.player.playerSpeed.y >= elm.enemy1Pos.y + 1 &&
            this.player.playerPos.x + this.player.playerSize.w >= elm.enemy1Pos.x &&
            this.player.playerPos.x <= elm.enemy1Pos.x + elm.enemy1Size.w) {

            game.enemies1.splice(elm, 1)
        }
    },

    enemy2HurtCollision(elm) {
        if (this.player.playerPos.y + this.player.playerSize.h <= elm.enemy2Pos.y + 1 &&
            this.player.playerPos.y + this.player.playerSize.h + this.player.playerSpeed.y >= elm.enemy2Pos.y + 1 &&
            this.player.playerPos.x + this.player.playerSize.w >= elm.enemy2Pos.x &&
            this.player.playerPos.x <= elm.enemy2Pos.x + elm.enemy2Size.w) {


            game.enemies2.splice(elm, 1)
    ...
    playerHurtCollision() {
        this.enemies1.forEach(elm => {
            if (this.player.playerPos.y + this.player.playerSize.h >= elm.enemy1Pos.y &&
                this.player.playerPos.y < elm.enemy1Pos.y + elm.enemy1Size.h &&
                this.player.playerPos.x + this.player.playerSize.w >= elm.enemy1Pos.x &&
                this.player.playerPos.x <= elm.enemy1Pos.x + elm.enemy1Size.w) {

                this.player.playerLifeCounter -= 5
            }
        })
        this.enemies2.forEach(elm => {
            if (this.player.playerPos.y + this.player.playerSize.h >= elm.enemy2Pos.y &&
    ...
    drawAll() {
        timer = setInterval(() => {
            ...
            //COWS
            this.enemies1.forEach((elm, idx) => {
                elm.draw(this.framesCounter)
                elm.move()
                this.enemy1HurtCollision(elm)
                elm.ballCollision()
                elm.enemy1Erase()
            })
